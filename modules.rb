...
module Modules
  ...
  def clear_file fname
    txt = file_to_text fname
    text_to_file txt.gsub("'", "`").gsub('"', ''), fname
  end

  def no_bom path
    system "CSharpNoBom/NoBom.exe #{path}"
  end
...
def mk_natasha
    ...
    f = file_to_arr(data_in_path + 'natasha.csv')
    f = f[6...f.size]
    f.each do |line|
  ...
  # cp1251 Запись в файл для кабинета
    ...
    def good_vec vec
      0.upto(vec.size-1) do |i|
        vec[i] = vec[i].chomp.gsub("'", "`").gsub('"','')
      end
    vec
  end
